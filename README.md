...
When no command is provided, the CLI still completes the startup checks first and then prints help.
...
If a daemon is already running and its version matches the current CLI version, `daemon run` reuses it.

If a daemon is already running but its version does not match the current CLI version, `daemon run` stops it and starts a new daemon from the current CLI binary.
...
After startup, the daemon uses the same upstream stdio MCP session to complete the MCP initialize handshake, list all tools, and write the result to `tools.json` in the same directory as the daemon socket.

If that MCP tool cache file is missing when `daemon run` checks startup state, the command waits until the daemon generates it before returning.
